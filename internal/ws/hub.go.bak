...
type Hub struct {
	...
	// clients maps each user's UUID to their active WebSocket
	// connection. Only one connection per user is supported;
...
// handleRegister adds a client to the registry. If the user
// already has an active connection, the old one is replaced
// (single-connection-per-user policy).
func (h *Hub) handleRegister(client *Client) {
	if existing, ok := h.clients[client.UserID]; ok {
		log.Printf("ws-hub: replacing existing connection for user %s", client.UserID)
		h.removeClientFromAllRooms(existing)
		close(existing.Send)
	}

	h.clients[client.UserID] = client
	log.Printf("ws-hub: registered user %s [total: %d]", client.UserID, len(h.clients))
}

// handleUnregister removes a client from the registry and all
// rooms. The Send channel is closed to signal the WritePump to
// exit. Only removes if the registered client matches (prevents
// a stale unregister from evicting a replacement connection).
func (h *Hub) handleUnregister(client *Client) {
	if current, ok := h.clients[client.UserID]; ok && current == client {
		h.removeClientFromAllRooms(client)
		delete(h.clients, client.UserID)
		close(client.Send)
		log.Printf("ws-hub: unregistered user %s [total: %d]", client.UserID, len(h.clients))
	}
}
...
// handleDirect delivers a message to a specific user. If the
// user is connected and their Send buffer has capacity, the
// message is queued. Otherwise, it is dropped with a log warning.
func (h *Hub) handleDirect(msg *DirectMessage) {
	...
	default:
		log.Printf("ws-hub: dropping direct message for slow client %s", msg.TargetUserID)
	}
}
